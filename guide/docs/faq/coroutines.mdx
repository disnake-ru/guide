...
Это содержание было взято непосредственно из [документации](https://docs.disnake.dev/en/latest/faq.html) и унаследованно от `discord.py `. Скорее всего, в будущем оно будет переписано.
