...
Как только пользователь введет информацию в мадольное окно, бот получит эти данные. В качестве примера эта информация
...
-   `timeout` - Время (в секундах), по истечению которого модальное окно будет удалено из кэша. По умолчанию -
    **600 секунд**.
-   `components` - Список компонентом, которые будут отображаться в модальном окне. Максимум **5 компонентов**.
...
пользовательского ввода в виде текста и не может использоваться вме модальных окон. Он имеет следующие атрибуты:
...
В качетсве альтернатиы вы можете
...
Выполняется, когда отправлка модального окна вызывает ошибку. Он возвращает ошибку, а также
...
-   Если стиль компонента текстового ввода не указан, используйтеся `short`.
-   По умолчанию все компоненты <DocsLink reference="disnake.ui.TextInput">TextInput</DocsLink> обязательны к заполнению.

## Лимиты

Существует несколько ограничений, о которых следует помнить при использовании моделей из-за ограничений API. Вот
