...
Бот должен быть добавлен в разрешением `applications.commands`, чтобы создавать слэш команды на сервере. Мы рекомендуем
...
Если вы укажете параметр `test_guilds` в `commands.Bot`, все слэш команды в вашем коде больше не будут являються
