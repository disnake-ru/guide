...
коллекцию команд/событий вашего бота (что полезно, когда список команд ващего бота становится довольно большим).
...
already exists a `main.py` file in your directory which initiates a `commands.Bot()` instance.
...
Последнее, что нужно добавить a этот файл, - это функция `setup`, чтобы disnake мог загружать ког, когда он будет
добавлен в `main.py`.
...
И это завершает использование когов в `disnake`! Теперь вы можете создать несколько кого для группировки и организации
