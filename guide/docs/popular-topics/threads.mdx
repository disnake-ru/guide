...
Присоединение к ветке и уход с неё требует у вас наличия объекта <DocsLink reference="disnake.Thread">Thread</DocsLink>,
с помощью которого Вы можете использовать методы <DocsLink reference="disnake.Thread.join">join()</DocsLink> и <DocsLink reference="disnake.Thread.leave">leave()</DocsLink> для соответствующих действий.
...
ветку можно создать, указав `type` в методе <DocsLink reference="disnake.TextChannel">метод create_thread()</DocsLink>
...
Приватная ветка быть создана только в объекте <DocsLink reference="disnake.TextChannel">TextChannel</DocsLink>. Тип,
...
этим [в документации](https://docs.disnake.dev/en/latest/api.html#disnake.on_thread_join).
