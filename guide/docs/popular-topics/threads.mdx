...
Присоединение и покидание ветки требует у вас наличия объекта <DocsLink reference="disnake.Thread">Thread</DocsLink>,
с помощью которого мы можем использовать методы <DocsLink reference="disnake.Thread.join">join()</DocsLink> и <DocsLink reference="disnake.Thread.leave">leave()</DocsLink> для соответствующих нужд.
